$(document).ready(function() {
    ...
    $('#horizontalBanner').on('click', function() {
        $('.addBlock').hide();
        renderBanner('horizontal');
        $('.swiper-container').show();
    })

    $('#verticalBanner').on('click', function() {
        $('.addBlock').hide();
        $('.swiper-container').show();
    ...
    $('#closeBanner').on('click', function() {
       ...
       $('.catalog').removeClass('hide-forever');

    })
    $(window).on('scroll', function() {
            if ($(this).scrollTop() > 50) {
                $('.catalog').show();
                // $('.top-btns').addClass('top-btns-fixed');
            } else {
                $('.catalog').hide();
                // $('.top-btns').removeClass('top-btns-fixed');
   ...
   htmlDecodeByRegExp:function (str){  
    ...
    function renderAll(addCon) {
        ...
        for (var i = 0; i < parseInt(localStorage.getItem('addNum')); i++) {
            ...
            var id = "id-" + iStr;
            addCon.innerHTML +=
                '<li id=' + id + ' class="list-group-item">' + '<div class="tip-p">' + '<xmp>' +
                localStorage.getItem(iStr) + '</xmp>' + '</div>' +
                '<span class="list-num">' + (i + 1) + '</span>' +
                scriptHtml1 +
                '</li>';
            //生成目录
            catalog += '<a title=' + HtmlUtil.htmlEncodeByRegExp(localStorage.getItem(iStr)) + ' name=' + id + ' class="catalog-a">' + (parseInt(iStr) + 1) + '</a>';



        }
        catalogDiv = '<div id="catalog" class="catalog">' + catalog + '</div>';
        $('body').append(catalogDiv);
        $('.catalog-a').on('click', function() {
            var id = $(this).attr('name');
            var currentItem = document.getElementById(id);
            currentItem.scrollIntoView();
            var scrollHeight = $('body').scrollTop() - 100;
    ...
    //渲染轮播信息
    function renderBanner(direction) {

        var swiperWrapper = $('#swiper-wrapper')[0];
        swiperWrapper.innerHTML = '';
        var swiperWrapperStr = '';
        //addNum 可以换成 
        //<div class="swiper-slide "><xml>slider1</xml></div>
        for (var i = 0; i < parseInt(localStorage.getItem('addNum')); i++) {
            //内容
            var iStr = i.toString();
            swiperWrapperStr +=
                '<div class="swiper-slide">' +
                '<xmp>' + (i + 1) + '、' + localStorage.getItem(iStr) + '</xmp>' +
                '</div>';
        }

        swiperWrapper.innerHTML = swiperWrapperStr;
